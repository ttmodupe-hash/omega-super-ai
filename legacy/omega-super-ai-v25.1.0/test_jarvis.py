#!/usr/bin/env python3
"""
Luqi AI v25.1.0 "JARVIS" — Comprehensive Test Suite
=====================================================
Tests all JARVIS integration components:
- ConversationMemory (SQLite persistence)
- ToolRegistry (dynamic tool registration)
- VoiceEngine (STT + TTS)
- JarvisAgent (core agent, chat, tool calling)
- FastAPI-compatible interface functions

Usage:
    python3 test_jarvis.py              # Standard run
    python3 test_jarvis.py --verbose    # Full tracebacks
    python3 test_jarvis.py --json       # Machine-readable output
"""
...
class TestRunner:
    ...
    def summary(self) -> bool:
        ...
        if self.json_mode:
            ...
            status = "HEALTHY — JARVIS integration ready" if self.failed == 0 else "DEGRADED — Fix failures before launch"
...
def test_memory(runner: TestRunner):
    ...
    try:
        mem = ja.ConversationMemory(db_path=db_path)

        def save_msg():
            mem.save_message("user", "Hello JARVIS", session_id="test_session")
        ...
        def search():
            results = mem.search_memories("JARVIS")
...
def test_voice_engine(runner: TestRunner):
    ...
    try:
        ...
        def speak_to_file():
            # Test file generation (TTS may not be available)
...
def test_builtin_tools(runner: TestRunner):
    ...
    def read_file():
        result = ja.read_file("backend/jarvis_agent.py", limit=5)
        assert "jarvis_agent.py" in result or "Lines" in result
        return result
    safe_test(runner, "Read file", read_file, lambda r: f"{r[:50]}...")

    def write_file():
        result = ja.write_file("data/test_write.txt", "Hello from JARVIS test")
    ...
    def run_code():
        result = ja.run_python_code("print(2 + 2)")
        assert "4" in result
        return result
    safe_test(runner, "Run Python code", run_code, lambda r: r.strip()[:20])

    def run_code_error():
        result = ja.run_python_code("print(undefined_var)")
        assert "error" in result.lower() or "Error" in result
        return result
    safe_test(runner, "Run Python code with error", run_code_error)

    def run_code_restricted():
        # Ensure restricted globals work
        result = ja.run_python_code("import os\nprint(os.getcwd())")
        # Should fail because __import__ is not in safe globals
...
def test_interface_functions(runner: TestRunner):
    ...
    def web_search_interface():
        result = ja.web_search("Python programming language")
    ...
    def store_fact_interface():
        result = ja.agent_store_fact("test_key", "test_value", "test_category")
        assert result["status"] == "success"
...
def main():
    parser = argparse.ArgumentParser(description="Luqi AI v25.1 JARVIS Test Suite")
    ...
    if not args.json:
        print(_c("\n" + "=" * 55, "bold"))
        print(_c("  Luqi AI v25.1.0 \"JARVIS\" — Test Suite", "bold"))
        print(_c("=" * 55, "bold"))
    ...
    if not args.json:
        print(f"\n{_c('Next steps:', 'bold')}")
        if healthy:
            print(f"  {_c('1.', 'green')} All {runner.passed} tests passed — JARVIS is ready")
            ...
            print(f"  {_c('2.', 'yellow')} Re-run: python3 test_jarvis.py")
